...
function formatTime(minutes, seconds) {
  const myMinutes = minutes.toString().padStart(2, '0');
  const mySeconds = seconds.toString().padStart(2, '0');
...
function countVowels(str) {
  ...
  const arr2 = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
...
function isPalindrome(str) {
  const prep1 = str
    ...
    .match(/[a-z]+/giu)
    .join('');
  const prep2 = prep1.split('').reverse().join('');
  return prep1.split('').join('') === prep2;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentArr.length; i += 1) {
    if (sentArr[i].length > maxLength) {
      maxLength = sentArr[i].length;
...
function encodeToRot13(str) {
  ...
  const movedAlpha = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  return str.replace(
    /[a-z]/gi,
    (letter) => movedAlpha[alphabet.indexOf(letter)]
  );
}
